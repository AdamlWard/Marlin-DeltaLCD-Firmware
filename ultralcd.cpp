...
void lcd_init()
{
    ...
    // AW: Adjustments for encoder that shorts to +5v on click (rather than shorts to ground)
    //WRITE(BTN_ENC,HIGH);
    //WRITE(BTN_ENC, LOW);
  #endif    
  #ifdef REPRAPWORLD_KEYPAD
...
void lcd_buttons_update()
{
  ...
  #if BTN_ENC > 0
    // AW: invert logic for my encoder
    //if((blocking_enc<millis()) && (READ(BTN_ENC)==0))
    if((blocking_enc<millis()) && (READ(BTN_ENC)==1))
        newbutton |= EN_C;
  #endif
  buttons=newbutton; 
...
// AW: Fix for delta printer and negative numbers -- https://github.com/ErikZalm/Marlin/issues/662
